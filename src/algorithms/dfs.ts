...
export function dfs(grid: Grid): AlgorithmResult {
  ...
  stack.push({ x: start.x, y: start.y, path: [start] })
  ...
  while (stack.length > 0) {
    ...
    peakMemory = Math.max(peakMemory, stack.length + visited.size)
...
export function* dfsGenerator(grid: Grid): Generator<AlgorithmStep, AlgorithmResult, unknown> {
  const start = grid.getStartPos()
  const end = grid.getEndPos()
